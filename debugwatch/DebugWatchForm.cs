...
namespace debugwatch
{
  public class DebugWatchForm : Form
  {
    ...
    private ulong address
    {
      get
      {
        return Convert.ToUInt64(this.AddressTextBox.Text.Trim().Replace("0x", ""), 16);
      }
    }
    ...
    private int length
    {
      get
      {
        return Convert.ToInt32(this.LengthTextBox.Text.Trim().Replace("0x", ""), 16);
      }
    }
    ...
    private string HexDump(byte[] bytes, int bytesPerLine = 16)
    {
      ...
      char[] charArray2 = (new string(' ', num3 - Environment.NewLine.Length) + Environment.NewLine).ToCharArray();
    ...
    private string[] GetDisassembly(ulong address, byte[] data)
    {
      ...
      foreach (Instruction instruction in new Disassembler(data, architecture, address, true, Vendor.Any, 0UL).Disassemble())
    ...
    private void ConnectButton_Click(object sender, EventArgs e)
    {
      ...
      if (this.ps4 != null && this.ps4.IsConnected)
        dialogResult = MessageBox.Show("You are already connected...\nWould you like to reconnect?", "Debug Watch", MessageBoxButtons.YesNo, MessageBoxIcon.Hand);
    ...
    private void AttachButton_Click(object sender, EventArgs e)
    {
      ...
      {
        int num = (int) MessageBox.Show("Please select a process in the list! Or press refresh then select!", "Debug Watch", MessageBoxButtons.OK, MessageBoxIcon.Hand);
      }
    }
    ...
    private void DebuggerInterruptCallback(
      ...
      dbregs dbregs)
    {
        var tabControlDelegate = new Func<object, object, int>((_param1, _param2) => this.TabControl.SelectedIndex = 0);
        var registersTextBoxDelegate = new Func<object, object, string>((_param1, _param2) =>
        ...
        var addressTextBoxDelegate = new Func<object, object, string>((_param1, _param2) => this.AddressTextBox.Text = "0x" + regs.r_rip.ToString("X"));
        var tryFindButtonDelegate = new Action<object, object>((_param1, _param2) => this.TryFindButton_Click((object) null, (EventArgs) null));
    ...
    private void ClearWatchPoints_Click(object sender, EventArgs e)
    {
      for (int index = 0; (long) index < (long) PS4DBG.MAX_WATCHPOINTS; ++index)
        this.ps4.ChangeWatchpoint(index, false, PS4DBG.WATCHPT_LENGTH.DBREG_DR7_LEN_1, PS4DBG.WATCHPT_BREAKTYPE.DBREG_DR7_EXEC, 0UL);
    }
    ...
    private void SetWatchpointButton_Click(object sender, EventArgs e)
    {
      ...
      this.ps4.ChangeWatchpoint((int) this.WatchpointNumericUpDown.Value, true, watchptLength, watchptBreaktype, this.address);
    }

    private void ClearWatchpointButton_Click(object sender, EventArgs e)
    {
      this.ps4.ChangeWatchpoint((int) this.WatchpointNumericUpDown.Value, false, PS4DBG.WATCHPT_LENGTH.DBREG_DR7_LEN_1, PS4DBG.WATCHPT_BREAKTYPE.DBREG_DR7_EXEC, 0UL);
    }
    ...
    {
      if (MessageBox.Show("Do you really want to reboot?", "Debug Watch", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) != DialogResult.Yes)
        return;
      this.ps4.Reboot();
    ...
    {
      this.ps4.WriteMemory(this.attachpid, this.address, ((MemoryViewByteProvider) this.MemoryHexBox.ByteProvider).Bytes.ToArray());
    }
    ...
    private void TryFindButton_Click(object sender, EventArgs e)
    {
      ...
      Instruction[] array = new Disassembler(this.ps4.ReadMemory(this.attachpid, address, 100), architecture, address, true, Vendor.Any, 0UL).Disassemble().ToArray<Instruction>();
    ...
    private void AutoPatchButton_Click(object sender, EventArgs e)
    {
      ...
      Instruction[] array = new Disassembler(this.ps4.ReadMemory(this.attachpid, uint64, 50), architecture, uint64, true, Vendor.Any, 0UL).Disassemble().ToArray<Instruction>();
    ...
    {
      if (MessageBox.Show("Do you really want to kill the process?\nThis may break your game until a reboot.", "Debug Watch", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) != DialogResult.Yes)
        return;
    ...
    private void NewScanButton_Click(object sender, EventArgs e)
    {
      ...
      {
        byte[] data = this.ps4.ReadMemory(this.attachpid, selectedEntry.start, (int) ((long) selectedEntry.end - (long) selectedEntry.start));
        if (data != null && data.Length != 0)
        {
          foreach (KeyValuePair<ulong, byte[]> keyValuePair in MemoryScanner.ScanMemory(selectedEntry.start, data, type, numArray, new MemoryScanner.CompareFunction(MemoryScanner.CompareEqual)))
            this.ScanDataGridView.Rows.Add((object) ("0x" + keyValuePair.Key.ToString("X")), (object) str, (object) keyValuePair.Value.ToString());
          GC.Collect();
        }
      }
    ...
    private void NextScanButton_Click(object sender, EventArgs e)
    {
      ...
      foreach (DataGridViewRow row in (IEnumerable) this.ScanDataGridView.Rows)
      {
        ...
        {
          case MemoryScanner.SCAN_TYPE.BYTE:
            if ((int) this.ps4.ReadMemory(this.attachpid, uint64, 1)[0] == (int) Convert.ToByte(this.ValueTextBox.Text))
            {
              flag = true;
              break;
            }
            break;
          case MemoryScanner.SCAN_TYPE.SHORT:
            if ((int)BitConverter.ToInt16(this.ps4.ReadMemory(this.attachpid, uint64, 2), 0) == (int) Convert.ToUInt16(this.ValueTextBox.Text))
            {
              flag = true;
              break;
            }
            break;
          case MemoryScanner.SCAN_TYPE.INTEGER:
            if ((int)BitConverter.ToInt32(this.ps4.ReadMemory(this.attachpid, uint64, 4), 0) == (int) Convert.ToUInt32(this.ValueTextBox.Text))
            {
              flag = true;
              break;
            }
            break;
          case MemoryScanner.SCAN_TYPE.LONG:
            if ((long)BitConverter.ToInt64(this.ps4.ReadMemory(this.attachpid, uint64, 8), 0) == (long) Convert.ToUInt64(this.ValueTextBox.Text))
            {
              flag = true;
              break;
            }
            break;
          case MemoryScanner.SCAN_TYPE.FLOAT:
            if (BitConverter.ToSingle(ps4.ReadMemory(this.attachpid, uint64, 4), 0) == (double) Convert.ToSingle(this.ValueTextBox.Text))
            {
              flag = true;
              break;
            }
            break;
          case MemoryScanner.SCAN_TYPE.DOUBLE:
            if (BitConverter.ToDouble(this.ps4.ReadMemory(this.attachpid, uint64, 8),0) == Convert.ToDouble(this.ValueTextBox.Text))
            {
              flag = true;
              break;
            }
    ...
    private void ScanDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
    {
      ...
      if (!(dataGridView.Columns[e.ColumnIndex] is DataGridViewButtonColumn) || e.RowIndex < 0)
        return;
      ulong uint64 = Convert.ToUInt64(dataGridView.Rows[e.RowIndex].Cells[0].ToString().Trim().Replace("0x", ""), 16);
      ulong typeLength = (ulong) MemoryScanner.GetTypeLength(dataGridView.Rows[e.RowIndex].Cells[1].ToString().Trim());
      PS4DBG.WATCHPT_LENGTH watchptLength = PS4DBG.WATCHPT_LENGTH.DBREG_DR7_LEN_1;
      switch ((long) typeLength - 1L)
      {
        ...
        case 1:
          ...
          goto case 2;
        case 2:
          this.ps4.ChangeWatchpoint((int) this.WatchpointNumericUpDown.Value, true, watchptLength, PS4DBG.WATCHPT_BREAKTYPE.DBREG_DR7_WRONLY, uint64);
          break;
        case 3:
          watchptLength = PS4DBG.WATCHPT_LENGTH.DBREG_DR7_LEN_4;
          goto case 2;
    ...
    {
      int num = (int) MessageBox.Show("This is really just a simple simple tool, you can do much much more with ps4debug if you want.\n\nDebug Watch, ps4debug, and jkpatch all created by golden\n\nShout out to all my testers, especially PS4 Guru, Shiningami, and Weysincha! Hit the link, to the left, and join the discord!", "Debug Watch", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
    }
    ...
    private void InitializeComponent()
    {
      ...
      this.ScanDataGridView.Columns.AddRange((DataGridViewColumn) this.AddressColumn, (DataGridViewColumn) this.TypeColumn, (DataGridViewColumn) this.ValueColumn, (DataGridViewColumn) this.WatchpointColumn);
      ...
      this.ScanDataGridView.CellContentClick += new DataGridViewCellEventHandler(this.ScanDataGridView_CellContentClick);
      ...
      this.KillProcessButton.Font = new Font("Microsoft Sans Serif", 9f, FontStyle.Regular, GraphicsUnit.Point, (byte) 0);
      ...
      this.StopButton.Font = new Font("Microsoft Sans Serif", 9f, FontStyle.Regular, GraphicsUnit.Point, (byte) 0);
      ...
      this.FilterProcessListCheckBox.CheckedChanged += new EventHandler(this.FilterProcessListCheckBox_CheckedChanged);
      ...
      this.SupportLinkLabel.LinkClicked += new LinkLabelLinkClickedEventHandler(this.SupportLinkLabel_LinkClicked);
